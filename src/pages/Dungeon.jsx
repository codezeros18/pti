...
import houseBg from '../assets/background/dungeon1.png';
...
const Dungeon = () => {
  const { eventMessage, updateStatus, money, addItem, inventory, addScore } = usePlayerStatus(); // <-- add addItem
  const [activityMessage, setActivityMessage] = useState(""); // <-- Add this
  const [position, setPosition] = useState({ top: 90, left: 60 }); // Top right spawn
  ...
  const handleLocationClick = (loc) => {
    const activity = locationActivities.dungeon.find(
  ...
  return (
    ...
    >
      ...
      <Activities location={activeLocation ? activeLocation.toLowerCase() : 'dungeon'} />
      <Map
