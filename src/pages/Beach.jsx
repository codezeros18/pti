...
import houseBg from '../assets/background/beach2.png';
import { locationActivities } from "../layout/locationActivities"; // adjust path if needed
import { useNavigate } from 'react-router-dom';

const INVENTORY_LIMIT = 3; // Add this near the top if not present
...
const Beach = () => {
  const { location, resetGame, eventMessage, updateStatus, money, addItem, inventory, addScore } = usePlayerStatus(); // add addItem
  const [position, setPosition] = useState({ top: 20, left: 60 }); // Top right spawn
  ...
  const handleLocationClick = (loc) => {
    const activity = locationActivities.beach.find(
  ...
  return (
    ...
    >
      ...
      <Activities location={activeLocation ? activeLocation.toLowerCase() : 'beach'} />
      <Map
